/*
  # Schéma initial de la base de données Silagora
  
  1. Tables
    - `profiles` - Profils utilisateurs
    - `souffles` - Messages géolocalisés
    - `suspended_tickets` - Tickets suspendus pour le système de don
  
  2. Sécurité
    - Activation de RLS sur toutes les tables
    - Politiques d'accès pour chaque table
  
  3. Fonctions
    - Fonctions pour la récupération des souffles
...
-- Create policies for profiles table (with IF NOT EXISTS)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Create policies for souffles table (with IF NOT EXISTS)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'souffles' AND policyname = 'Anyone can read non-expired souffles'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'souffles' AND policyname = 'Authenticated users can create souffles'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'souffles' AND policyname = 'Users can update own souffles'
    ) THEN
...
-- Create policies for suspended_tickets table (with IF NOT EXISTS)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'suspended_tickets' AND policyname = 'Anyone can read unclaimed tickets'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'suspended_tickets' AND policyname = 'Authenticated users can create tickets'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'suspended_tickets' AND policyname = 'Users can update tickets to claim them'
    ) THEN
